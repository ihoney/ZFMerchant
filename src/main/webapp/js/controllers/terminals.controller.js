...
var terminalController = function ($scope, $http, LoginService) {
	...
	$scope.getInfo = function () {
      ...
      //获取总页数
      /*if(($scope.totalSize%$scope.pageNum) == 0){
    	  $scope.totalPage = $scope.totalSize/$scope.pageNum;
      }
      if(($scope.totalSize%$scope.pageNum) > 0){
    	  $scope.totalPage = $scope.totalSize/$scope.pageNum+1;
      }*/
      ...
      if ($scope.indexPage > 1 && $scope.indexPage < $scope.totalPage) {
    	  if($scope.totalPage<=10){
    		  for(var i=0;i<$scope.totalPage;i++){
    			  $scope.pages[i] = $scope.indexPage + (i-1);
    		  }
