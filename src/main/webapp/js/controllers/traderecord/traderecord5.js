...
// 交易流水
var traderecord5Module = angular.module("traderecord5Module", []);
var traderecord5Controller = function($scope, $http, LoginService) {
	...
	$scope.getTradeRecordTotal = function() {// 交易笔数和交易总额
		var query = 5 + "/" + $scope.terminalNumber.serialNum + "/" + $scope.startTime + "/" + $scope.endTime;
	...
	$scope.list = function() {
		var query = 5 + "/" + $scope.terminalNumber.serialNum + "/" + $scope.startTime + "/" + $scope.endTime + "/" + $scope.indexPage + "/" + $scope.rows;
...
traderecord5Module.controller("traderecord5Controller", traderecord5Controller);